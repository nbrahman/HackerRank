...
Each test case consists of two lines. The first line has two space-separated integers, N (students in the class) and K (the cancelation threshold). The second line contains N space-separated integers (a<0>,a<1>,...a<n-1>) describing the arrival times for each student.
...
1<=N<=1000
1<=K<=N
-100<=a<i><=100,where i is in range [1,N]
...
def AngryProfessorSolution(k, a):
    t=sum(i<=0 for i in a)
    if t>=k:
        print('NO')
    else:
        print('YES')
